...
if df is not None:
    ...
    if COL_L in df.columns and COL_Q in df.columns:
        ...
        filas_html = "".join([f"""
            <tr style="border: 1px solid black; font-weight: bold; font-size: 15px;">
                <td style="text-align: left; padding: 12px; border: 1px solid black; background-color: white;">{name}</td>
        ...
        <div style="font-family: Arial; text-align: center;">
            <h2 style="margin-bottom: 15px;">Cuadro Tiempo de Atención Despachadores</h2>
            <table style="width: 100%; border-collapse: collapse; text-align: center; border: 2px solid black;">
                <thead>
                    <tr style="background-color: white;">
                        <th style="border: 1px solid black; padding: 12px; width: 30%;">Supervisor Despacho</th>
                        <th style="background-color: #FFC0CB; border: 1px solid black; width: 14%;">Alerta (>1h)</th>
                        <th style="background-color: #FFF3CD; border: 1px solid black; width: 14%;">Aceptable (40-60m)</th>
                        <th style="background-color: #D4EDDA; border: 1px solid black; width: 14%;">Excelente (<40m)</th>
                        <th style="background-color: #4472C4; color: white; border: 1px solid black; width: 14%;">Total</th>
                        <th style="background-color: red; color: white; border: 1px solid black; padding: 12px; width: 14%;">% viajes demorados</th>
                    </tr>
                </thead>
                <tbody>{filas_html}</tbody>
                <tfoot style="background-color: #FFFF00; font-weight: bold; font-size: 18px;">
                    <tr><td style="text-align: right; padding: 12px; border: 1px solid black;">Total Semana</td>
                    <td colspan="3" style="border: 1px solid black;"></td>
                    <td style="border: 1px solid black;">{t_total}</td>
                    <td style="border: 1px solid black; background-color: white; color: red;">{pct_gral}%</td></tr>
                </tfoot>
            </table>
        </div>"""
        
        # --- CÁLCULO DE ALTURA DINÁMICA PARA LA TABLA ---
        # Aproximadamente 52px por fila + 150px de encabezado y pie
        altura_dinamica = (len(res) * 52) + 160
    ...
    # --- 🏢 FOOTER SBI LATAM ---
    st.markdown("""<style>
        .footer { width: 100%; color: #555555; text-align: center; padding: 30px 0px; border-top: 1px solid #e6e6e6; margin-top: 50px; }
